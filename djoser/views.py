...
class RegistrationView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if settings.SEND_ACTIVATION_EMAIL:
            email.ActivationEmail(self.request, context).send_to(recipients)
        elif settings.SEND_CONFIRMATION_EMAIL:
            email.ConfirmationEmail(self.request, context).send_to(recipients)
...
class PasswordResetView(utils.ActionViewMixin, generics.GenericAPIView):
    ...
    def send_password_reset_email(self, user):
        context = {'user': user}
        recipients = [get_user_email(user)]
        email.PasswordResetEmail(self.request, context).send_to(recipients)
